...
metadata_cols <- colnames(microbiome_data[,c(1,2)])
  ...
  for (group1 in groups){
    ...
    for (group2 in groups){
      if (group1 != group2 & !(group2 %in% used_groups) ){
        ...
        write.csv(lefser_resultsTP, paste(output, "/lefse/csvs/lefseresults",column,group1,group2,".csv",sep=""), row.names = F)
        #png(paste(output, "/lefse/plots/lefseresults",column,group1,group2,".png",sep=""), height = 900)
        tiff(paste(output, "/lefse/plots/lefseresults",column,group1,group2,".tiff",sep=""), units="in", width=7, height=10, res=300)
	      lefserPlot(lefser_resultsTP,trim.names=FALSE) %>% print()
        dev.off()
