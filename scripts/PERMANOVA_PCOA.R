...
writeData(wb, "Group Site independently", permanova_independent, rowNames = TRUE)

addWorksheet(wb, "Group Site interaction")
writeData(wb, "Group Site interaction", permanova_interaction, rowNames = TRUE)

addWorksheet(wb, "Group_Condition-Site")
writeData(wb, "Group_Condition-Site", permanova_interaction_group_condition, rowNames = TRUE)
...
png(paste(output,"/permanova/pcoa_interaction.png",sep=""))
perm_df <- permanova_interaction["Group:Site",]
pcoa(data_cols, metadata, "Interaction between Group and Site", "Group_Site")
dev.off()

png(paste(output,"/permanova/pcoa_site.png",sep=""))
perm_df <- permanova_interaction["Site",]
pcoa(data_cols, metadata, "Site", "Site")
dev.off()


png(paste(output,"/permanova/pcoa_group.png",sep=""))
perm_df <- permanova_interaction["Group",]
...
png(paste(output,"/permanova/pcoa_Group-Condition-Site.png",sep=""))
perm_df <- permanova_interaction_group_condition["Group_Condition:Site",]
pcoa(data_cols, metadata, "Group-Condition-Site", "Group_Condition_Site")
dev.off()
...
tongue_metadata <- metadata[metadata$Group_Site == "XS-T" | metadata$Group_Site == "NX-T",]
