...
microbiome_data <- relocate(microbiome_data, Group_Condition)
    ...
    for (group1 in groups){
      ...
      for (group2 in groups){
        if (group1 != group2 & !(group2 %in% used_groups) ){
          ...
          dir.create(file.path( paste(output,"/wilcox/plots", sep="") ), showWarnings = FALSE)
...
calculate_wilcox(microbiome_data, colnames(microbiome_data)[2]) ## Group
calculate_wilcox(microbiome_data, colnames(microbiome_data)[3]) ## Condition
